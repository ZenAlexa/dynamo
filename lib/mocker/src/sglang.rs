...
impl LogprobOptions {
    pub fn new(
        ...
        let top_logprobs_num = usize::try_from(top_logprobs_num)
            .map_err(|_| format!("top_logprobs_num must be between 0 and {MAX_TOP_LOGPROBS}"))?;
        if top_logprobs_num > MAX_TOP_LOGPROBS {
            return Err(format!(
                "top_logprobs_num must be between 0 and {MAX_TOP_LOGPROBS}"
            ));
        }
        let logprob_start_len = i32::try_from(logprob_start_len)
            .map_err(|_| "logprob_start_len must fit in int32".to_string())?;
...
impl Default for LogprobOptions {
    fn default() -> Self {
        Self {
...
pub struct ResponseMetadata {
    request_id: String,
    prompt_tokens: usize,
    prompt_tokens_for_logprobs: Option<Vec<u32>>,
...
impl ResponseMetadata {
    pub fn new(
        ...
        let prompt_tokens_for_logprobs = (logprob_options.return_logprob
            && logprob_options.logprob_start_len >= 0)
            .then(|| prompt_tokens.to_vec());
        Self {
            request_id: request_id.into(),
            prompt_tokens: prompt_tokens.len(),
            prompt_tokens_for_logprobs,
            logprob_options,
        }
    }
    ...
    fn insert_prompt_logprobs(&self, meta_info: &mut Map<String, Value>) {
        let Some(prompt_tokens) = self.prompt_tokens_for_logprobs.as_deref() else {
            return;
        };
        let start = (self.logprob_options.logprob_start_len as usize).min(prompt_tokens.len());
        let prompt_tokens = &prompt_tokens[start..];
