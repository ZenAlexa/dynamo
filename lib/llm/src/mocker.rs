    ...
    for MockerExecutionContext
{
    async fn generate(
        ...
        let native_sglang =
            sglang::ResponseAdapter::from_request(&request, ctx.id()).map_err(Error::from)?;
            ...
            .then(|| {
                request
                    ...
                    .or_else(|| {
                        ...
                        let adapter = native_sglang.as_ref()?;
                        self.response_replay_table.as_ref()?;
                        Some(adapter.replay_key().to_string())
                    })
            })
            .flatten();
        ...
        let stream = UnboundedReceiverStream::new(stream_rx).map(move |mut output| {
            if let Some(adapter) = native_sglang.as_ref() {
                completion_tokens = completion_tokens.saturating_add(output.token_ids.len());
                adapter.adapt(&mut output, completion_tokens);
            }
